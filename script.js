// ====================== WEB LEYLAPARISDEV - VERSIÓN MEJORADA ======================
...
let participantsCount = 1247; // Contador inicial (se puede aumentar)
...
document.getElementById("submitSurvey").addEventListener("click", () => {
  ...
  // Notificación + confeti rosa
  createConfetti();
  
  const notif = document.createElement("div");
  notif.style.cssText = "position:fixed;bottom:40px;left:50%;transform:translateX(-50%);background:linear-gradient(45deg,#ff1493,#ff69b4);color:white;padding:25px 55px;border-radius:30px;font-size:1.45rem;box-shadow:0 0 70px #ff1493;z-index:99999;text-align:center;";
  notif.innerHTML = `🎉 ¡Gracias ${name}!<br>Tu participación ha sido enviada.<br>Leyla la leerá pronto 💖`;
  ...
  for (let i = 0; i < 80; i++) {
    setTimeout(() => {
      ...
      confetti.style.cssText = `
        ...
        background:${Math.random() > 0.5 ? '#ff69b4' : '#ffb6c1'};
...
floatBtn.innerHTML = `<i class="fas fa-heart"></i> Quiero participar`;
floatBtn.style.cssText = `
  position:fixed; bottom:30px; right:25px; 
  background:linear-gradient(45deg,#ff1493,#ff69b4); 
  color:white; border:none; border-radius:50px; 
  padding:18px 28px; font-size:1.2rem; box-shadow:0 0 40px #ff1493;
  z-index:999; cursor:pointer; display:flex; align-items:center; gap:10px;
`;
floatBtn.onclick = () => document.querySelector(".survey").scrollIntoView({ behavior: "smooth" });
...
loadVideos();
setRandomMotivation();
setInterval(setRandomMotivation, 25000); // Cambia cada 25 segundos
