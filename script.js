...
document.getElementById("submitSurvey").addEventListener("click", () => {
  ...
  notif.style.cssText = "position:fixed;bottom:40px;left:50%;transform:translateX(-50%);background:linear-gradient(45deg,#ff1493,#ff69b4);color:white;padding:25px 60px;border-radius:30px;font-size:1.45rem;box-shadow:0 0 70px #ff1493;z-index:99999;";
...
function createConfetti() {
  for (let i = 0; i < 100; i++) {
    setTimeout(() => {
      ...
      c.style.cssText = `position:fixed;top:-10px;left:${Math.random()*100}vw;width:10px;height:10px;border-radius:50%;background:${Math.random()>0.5?'#ff69b4':'#ffb6c1'};z-index:99998;animation:fall ${Math.random()*4+3}s linear forwards;`;
